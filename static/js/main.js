document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayResults(data) {
    ...
    document.getElementById("brandDescription").textContent =
      data.brand_description || "No description available.";
    ...
    if (score >= 85) {
      scoreCircle.className =
        "w-20 h-20 rounded-full border-4 border-cyan-500 flex items-center justify-center mx-auto";
    } else if (score >= 70) {
      scoreCircle.className =
        "w-20 h-20 rounded-full border-4 border-teal-500 flex items-center justify-center mx-auto";
    } else if (score >= 50) {
      scoreCircle.className =
        "w-20 h-20 rounded-full border-4 border-yellow-500 flex items-center justify-center mx-auto";
    } else {
      scoreCircle.className =
        "w-20 h-20 rounded-full border-4 border-red-500 flex items-center justify-center mx-auto";
    }
    ...
    if (data.key_values && data.key_values.length > 0) {
      data.key_values.forEach((value) => {
        ...
        badge.className =
          "bg-opacity-20 bg-primary text-white px-3 py-1 rounded-full text-sm";
        badge.textContent = value;
        keyValues.appendChild(badge);
      });
    } else {
      keyValues.innerHTML =
        '<span class="text-gray-500">No key values available</span>';
    }
    ...
    if (data.social_links && data.social_links.length > 0) {
      data.social_links.forEach((social) => {
        ...
        switch (social.platform) {
          case "facebook":
            icon.className = "fab fa-facebook-f mr-2";
            break;
          case "twitter":
            icon.className = "fab fa-twitter mr-2";
            break;
          case "instagram":
            icon.className = "fab fa-instagram mr-2";
            break;
          case "linkedin":
            icon.className = "fab fa-linkedin-in mr-2";
            break;
          case "youtube":
            icon.className = "fab fa-youtube mr-2";
            break;
          case "tiktok":
            icon.className = "fab fa-tiktok mr-2";
            break;
          case "pinterest":
            icon.className = "fab fa-pinterest-p mr-2";
            break;
          default:
            icon.className = "fas fa-link mr-2";
        }
      ...
      socialLinks.innerHTML =
        '<span class="text-gray-500">No social media links detected</span>';
    }
    ...
    if (data.brand_story) {
      ...
      brandStoryContent.innerHTML =
        '<p class="text-gray-500">No brand story available.</p>';
    }
    ...
    if (data.keywords && data.keywords.length > 0) {
      data.keywords.forEach((keyword) => {
        const badge = document.createElement("span");
        badge.className =
          "bg-gray-800 text-white px-3 py-1 rounded-full text-sm";
        badge.textContent = keyword;
        keyKeywords.appendChild(badge);
      });
    } else {
      keyKeywords.innerHTML =
        '<span class="text-gray-500">No keywords available</span>';
    }
    ...
    if (data.visual_profile && data.visual_profile.tone_indicators) {
      ...
      toneAnalysis.innerHTML =
        '<p class="text-gray-500">No tone analysis available</p>';
    }
    ...
    if (data.visual_profile && data.visual_profile.color_palette) {
      ...
      colorPalette.innerHTML =
        '<p class="text-gray-500">No color palette available</p>';
    }
    ...
    if (data.visual_profile && data.visual_profile.font_style) {
      ...
      typography.innerHTML =
        '<p class="text-gray-500">No typography information available</p>';
    }
