...
def analyze_website():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Extracted content for brand: {website_content.get('brand_name', default_brand_name)}")
            
            # Update result with website content
            result['brand_name'] = website_content.get('brand_name', default_brand_name)
            result['brand_description'] = website_content.get('description', result['brand_description'])
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Extracted content from {len(social_content)} social platforms")
        ...
        try:
            ...
            logger.info(f"Content analysis complete. Found {len(analysis.get('keywords', []))} keywords")
