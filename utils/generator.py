...
def generate_brand_story(brand_name, description, analysis, social_content):
    ...
    try:
        ...
        if not keywords:
            keywords = ["professional", "quality", "service", "experience", "innovation"]
        ...
        brand_intro = generate_brand_intro(brand_name, description, key_values, dominant_tone)
        ...
        brand_conclusion = generate_brand_conclusion(brand_name, key_values, dominant_tone)
...
def generate_brand_values(brand_name, key_values, tone):
    ...
    try:
        ...
        if len(value_points) < 3:
            default_values = ['Quality', 'Innovation', 'Customer Focus', 'Excellence', 'Integrity']
    ...
    try:
        ...
        if secondary_tones:
            if len(secondary_tones) == 1:
                secondary_tone_text = f"This is balanced with notes of {secondary_tones[0]}."
            else:
                secondary_tone_text = f"This is balanced with notes of {' and '.join(secondary_tones)}."
        ...
        if keywords:
            ...
            selected_keywords = ["quality", "professional", "service", "excellence", "innovation"]
...
def generate_social_presence(brand_name, social_content):
    ...
    try:
        ...
        has_visual = any(p.get('platform', '').lower() in ['instagram', 'tiktok', 'pinterest'] for p in social_content)
        has_professional = any(p.get('platform', '').lower() in ['linkedin'] for p in social_content)
        has_content = any(p.get('platform', '').lower() in ['youtube', 'medium'] for p in social_content)
        
        strategy_points = []
        
        if has_visual:
            strategy_points.append("- Strong visual identity and aesthetic appeal to engage visually-oriented audiences")
        if has_professional:
            strategy_points.append("- Focus on industry expertise and professional networking")
        if has_content:
            strategy_points.append("- Investment in longer-form educational and informative content")
        if len(social_content) >= 3:
            strategy_points.append("- Multi-platform approach to reach diverse audience segments")
        else:
            strategy_points.append("- Targeted platform selection to focus resources on key audience channels")
        
        # If no specific insights, add a general one
        if not strategy_points:
            strategy_points.append("- Strategic platform selection aligned with the brand's communication goals and target audience")
        
        # Combine into social media section
        social_presence = f"{social_intro}\n\n" + "\n".join(platform_details) + f"{strategy_insights}\n\n" + "\n".join(strategy_points)
...
def generate_visual_profile(analysis):
    ...
    try:
        ...
        selected_palette = color_palettes.get(dominant_tone, color_palettes['informative'])
    ...
    try:
        ...
        if brand_name:
            for social in social_content:
                ...
                if (brand_name.lower() not in content and 
                    brand_name.lower().replace(' ', '') not in content.replace(' ', '')):
